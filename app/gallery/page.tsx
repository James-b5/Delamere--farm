...
export default function GalleryPage() {
  ...
  useEffect(() => {
    // Public endpoint to fetch products with media
    fetch("/api/products")
      .then((res) => {
  ...
  // Gather all images and videos across products
          ...
          <>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {allVideos.map((vid, idx) => (
                ...
                >
                  <video src={vid.src} controls preload="metadata" className="w-full h-64 object-cover" />
                </div>
              ))}
            </div>
          </>
